...
//    this->ID="admin";
//    ui->label_ID->setText(this->ID);//登录界面回去用户ID
...
void Func::audioCtrl(QString text)
{
    ...
    qDebug()<<"Speech recognition results:"<<strTmp<<endl;
    //识别音乐
    emit send_music_name(strTmp);
    //功能
    if(strTmp=="起床"){
        emit send_wake();
    }
...
//ID
void Func::set_ID(QString setID)
{
    this->ID=setID;
    ui->label_ID->setText(setID);
}
